...
class Command(BaseCommand):
    # 帮助信息，描述命令的功能
    help = '修改数据库表字段的注释，使其与模型的verbose_name一致，并设置create_datetime和update_datetime字段的默认值和触发器'
        ...
        try:
            with connection.cursor() as cursor:
                ...
                if "mysql" in connection_type_info:
                    ...
                    self.setup_datetime_fields(cursor, custom_models)
                    processed = True
    ...
    def setup_datetime_fields(self, cursor, custom_models):
        # 设置create_datetime和update_datetime字段的默认值和触发器
        for model_obj in custom_models:
            table_name = model_obj._meta.db_table
            fields = model_obj._meta.fields
            has_create_datetime = any(f.name == 'create_datetime' for f in fields)
            has_update_datetime = any(f.name == 'update_datetime' for f in fields)

            if has_create_datetime:
                try:
                    cursor.execute(
                        f"ALTER TABLE `{table_name}` MODIFY COLUMN `create_datetime` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) COMMENT '创建时间';"
                    )
                    self.stdout.write(f"已为表 {table_name} 的 create_datetime 字段设置默认值")
                except Exception as e:
                    logger.error(f"未能为表 {table_name} 的 create_datetime 字段设置默认值: {e}")

            if has_update_datetime:
                try:
                    cursor.execute(
                        f"ALTER TABLE `{table_name}` MODIFY COLUMN `update_datetime` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6) COMMENT '更新时间';"
                    )
                    self.stdout.write(f"已为表 {table_name} 的 update_datetime 字段设置默认值和触发器")
                except Exception as e:
                    logger.error(f"未能为表 {table_name} 的 update_datetime 字段设置默认值和触发器: {e}")
